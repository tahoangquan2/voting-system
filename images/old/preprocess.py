...
for page_num, group in groupby(df.iterrows(), key=lambda x: x[1]['page']):
    image_path = os.path.join('images', f'page_{page_num}.jpeg')
    ...
    for crop_idx, (_, row) in enumerate(group, start=1):
        try:
            bbox = np.array(ast.literal_eval(row['bounding_boxes']), dtype="float32")
            warped = four_point_transform(image, bbox)

            output_path = os.path.join('output', f'page_{page_num:03d}_{crop_idx:02d}.jpg')
